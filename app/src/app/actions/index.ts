...
export async function createEvent(
  ...
  try {
    ...
    const DeadlineDate =
      (formData.get("registrationDeadline") as string) || date;
    const DeadlineDateTime = `${DeadlineDate}T23:59:00`;
    const minParticipantsPerTeam = Number(
      formData.get("minParticipantsPerTeam"),
    );
    const maxParticipantsPerTeam = Number(
      formData.get("maxParticipantsPerTeam"),
    );
...
export async function getIndividualEventDetails(
  id: string,
): Promise<getIndividualEventDetailsProp | null> {
...
export async function teamRegister(
  ...
  try {
    ...
    return {
      message: (error as any).message || "Failed to register for the team",
      success: false,
    };
  }
}
